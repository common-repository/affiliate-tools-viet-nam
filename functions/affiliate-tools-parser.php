...
include_once( dirname( __FILE__ ) . '/affiliate-factory/affiliate-factory.php' );

class AffiliateToolsParser extends AffiliateToolsBase {
    
    var $debug_log = array();
    
    function output_debug() {
        
        echo "\n<!--affiliate-tools debug:\n" . implode("\n\n", $this->debug_log) . "\n-->";
    }
    ...
    function encode_link($url)
    {
        ...
        if ($this->options['aff_base64']) {
            ...
            if (is_null($result) && strpos($wpdb->last_error, "doesn't exist"))//no table found
            {
                ...
                echo '<div class="error">' . __('Failed to make masked link. MySQL link table does not exist. Trying to create table.', 'affiliate-tools') . '</div>';
                $sql2 = 'CREATE TABLE ' . $wpdb->prefix . 'masklinks(`id` INT UNSIGNED NOT NULL AUTO_INCREMENT,`url` VARCHAR(255),  PRIMARY KEY (`id`))';
                $res = $wpdb->query($sql2);
                if (!$res) {
                    echo '<div class="error">' . __('Failed to create table. Please, check mysql permissions.', 'affiliate-tools') . '</div>';
                    $this->debug_info(__('Failed SQL: ', 'affiliate-tools') . '<br>' . $sql2 . '<br>' . __('Error was:', 'affiliate-tools') . '<br>' . $wpdb->last_error);
                } else {
                    echo '<div class="updated">' . __('Table created.', 'affiliate-tools') . '</div>';
                    $wpdb->query($sql);
                }
            } elseif (is_null($result)) {
                $this->debug_info(__('Failed SQL: ', 'affiliate-tools') . '<br>' . $sql . '<br>' . __('Error was:', 'affiliate-tools') . '<br>' . $wpdb->last_error);
            }
            if (!$result) {
    ...
    function AffiliateToolsParser() #constructor
    {
        $this->load_options();
        $this->set_filters();
    ...
    function check_redirect()#checking if it is redirect page
    {
        ...
        $p = strpos($_SERVER['REQUEST_URI'], '/' . $this->options['aff_LINK_SEP'] . '/');

        if( isset($_REQUEST[$this->options['aff_LINK_SEP']]) ){
            $goto = $_REQUEST[$this->options['aff_LINK_SEP']];
        } elseif ($p !== FALSE) {
            $goto = substr($_SERVER['REQUEST_URI'], $p + strlen($this->options['aff_LINK_SEP']) + 2);
        }
        ...
        /*else {
            ...
            $url = explode('/', $url);
            if ($url[sizeof($url) - 2] == $this->options['aff_LINK_SEP'])
    ...
    function show_referer_warning() {
        ...
        ?>
        <html>
            <head><title><?php _e('Redirecting...', 'affiliate-tools'); ?></title>
        ...
        <div align="center" style="margin-top: 15em;">
            <?php
            echo __('You have been redirected through this website from a suspicious source. We prevented it and you are going to be redirected to our ', 'affiliate-tools') . '<a href="' . get_home_url() . '">' . __('safe web site.', 'affiliate-tools') . '</a>';?>
        </div>
        </body></html><?php die();
    }
    ...
    function add_stats_record($url) {
        ...
        $sql = 'INSERT INTO ' . $wpdb->prefix . 'links_stats VALUES("", "'.$url.'" , NOW() , "'.$title.'" )';
        $res = $wpdb->query($sql);
        ...
        parent::activate();
        $this->debug_info(__('Failed SQL: ', 'affiliate-tools') . '<br>' . $sql . '<br>' . __('Error was:', 'affiliate-tools') . '<br>' . $wpdb->last_error);
        echo '<div class="error">' . __('Failed to save statistic data. Please deactivate and reactivate to fix this error.', 'affiliate-tools') . '</div>';
    }
    ...
    function redirect($url)
    {
        ...
        if ($this->options['aff_restrict_referer']) {
        ...
        $this->show_redirect_page(aff_get_affiliate_url($url));
    }
    ...
    <head><title><?php _e('Redirecting...', 'affiliate-tools'); ?></title>
        ...
        if ($this->options['aff_redtime'])
            echo $this->options['aff_redtime'];
        else echo '0';
        echo '; url=' . $url . '" />'; ?>
    </head>
    <body style="margin:0;">
    <div align="center" style="margin-top: 15em;">
        <?php
        if ($this->options['aff_redtxt'] && $url)
            echo str_replace('LINKURL', $url, $this->options['aff_redtxt']);
        elseif ($url)
            echo __('You were going to the redirect link, but something did not work properly.<br>Please, click ', 'affiliate-tools') . '<a href="' . $url . '">' . __('HERE ', 'affiliate-tools') . '</a>' . __(' to go to ', 'affiliate-tools') . $url . __(' manually. ', 'affiliate-tools');
        else
            _e('Sorry, no url redirect specified. Can`t complete request.', 'affiliate-tools');?>
    </div>
    </body></html><?php die();
    }
    ...
    function filter($content)
    {
        ...
        if (function_exists('is_feed') && is_feed() && !$this->options['aff_mask_rss'] && !$this->options['aff_mask_rss_comment']) {
    ...
    function fullmask_begin()
    {
        ...
        if (!$a)
            echo '<div class="error">' . __('Can not get output buffer!') . __('AffiliateToolsBase Can`t use output buffer. Please, disable full masking and use other filters.', 'affiliate-tools') . '</div>';
        if ($this->options['aff_debug'])
    ...
    function fullmask_end($text)
    {
        ...
        if (!$text)
            $r .= '<div class="error">' . __('Output buffer empty!') . __('AffiliateToolsBase Can`t use output buffer. Please, disable full masking and use other filters.', 'affiliate-tools', 1) . '</div>';
        else {
    ...
    function set_filters() {
        ...
        if ($this->options['aff_fullmask']) {
            ...
            add_filter('AffiliateToolsBase', array($this, 'filter'), 99);
        }
    }
}
